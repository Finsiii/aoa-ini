...
async function pagi(nw, x, data) {
    ...
    return await KhaerulZx.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(nw, x, data) {
    let row = [];
    let ment = [];
...
async function malam(nw, x, data) {
...
async function skill(nw, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await KhaerulZx.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, nw, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await KhaerulZx.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(nw, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(nw, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nw, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(nw, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nw, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(nw, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nw, data);
            break;
        } else {
            await skill(nw, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, nw, data);
}

async function run_vote(nw, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nw, data);
            break;
...
async function run_malam(nw, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nw, data);
            break;
...
async function run_pagi(nw, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, nw, data);
            break;
